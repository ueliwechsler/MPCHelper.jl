...
"""
        lmax_mask(z, v, P, α, c_x, c_u, K)

    Calculate `lmax(z,v) = lmax(z, v, P, α, c_x, c_u, K)` for an ellipsoidal set
    `Ω = {x ∈ ℝⁿ | x'Px ≤ α}`, the linear system `l(x,u)= lxᵀx + luᵀu` and the
    linear feedback gain `K`.
    `lmax(z,v) = l(z,v) +  max (lx + K'lu)'ω s.t. ω ∈ Ω`. """
function lmax_mask(z, v, P::AbstractMatrix, α::Real, c_x, c_u, K)
    l(x,u) = dot(c_x, x) + dot(c_u, u)
    γ = c_x + K'*c_u
    hΩ = hΩ_opt(γ, P, α)
    return l(z,v) + hΩ
    # ω = ω_opt(γ, P, α)
...
function lmax_mask(z, v, Ω::LazySet, c_x, c_u, K)
    l(x,u) = dot(c_x, x) + dot(c_u, u)
    γ = c_x + K'*c_u
    hΩ = ρ(vec(γ), Ω)
    return l(z,v) + hΩ
...
"""
        max_cost_diff_over_set(P, α, c_x, c_u, K)
...
function max_cost_diff_over_set(P::AbstractMatrix, α::Real, c_x, c_u, K)
    l(x,u) = dot(c_x, x) + dot(c_u, u)
    γ = c_x + K'*c_u
...
function max_cost_diff_over_set(Ω::LazySet, c_x, c_u, K)
    l(x,u) = dot(c_x, x) + dot(c_u, u)
    γ = c_x + K'*c_u
    hΩmax = ρ(vec(γ), Ω)
    hΩmin = ρ(vec(-γ), Ω)
